...
public class TestCloneSnapshotFromClient {
  ...
  public void testCloneSnapshotAfterSplittingRegion() throws IOException, InterruptedException {
    ...
    // Get a state of the split parent region of the cloned table
    List<RegionInfo> regionInfosOfClonedTable = MetaTableAccessor
      .getTableRegions(admin.getConnection(), clonedTableName, false);

    RegionInfo regionInfoOfSplitRegion = regionInfosOfClonedTable.stream()
      .filter(ri -> ri.isOffline() && (ri.isSplit() || ri.isSplitParent()))
      .findAny()
      .orElseThrow(() -> new IllegalStateException(
        "The split parent region of the cloned table doesn't exist. Why?"));

    RegionState.State stateOfSplitRegion = TEST_UTIL.getHBaseCluster().getMaster()
      .getAssignmentManager().getRegionStates().getRegionState(regionInfoOfSplitRegion).getState();

    // The state of the split parent region should be SPLIT
    assertEquals(RegionState.State.SPLIT, stateOfSplitRegion);
